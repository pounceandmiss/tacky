...
test messagestore-prev-latest-chain {get latest annotates prev: empty for first, chained after} \
    {*}$ms_common \
    -body {
        ms_batch [list \
            [ms_msg timestamp 100 body a] \
...
test messagestore-prev-before-chain {get before annotates prev with DB predecessor for first element} \
    {*}$ms_common \
    -body {
        ms_batch [list \
            [ms_msg timestamp 100 body a] \
...
test messagestore-prev-after-chain {get after annotates prev correctly} \
    {*}$ms_common \
    -body {
        ms_batch [list \
            [ms_msg timestamp 100 body a] \
...
test messagestore-prev-single-message {single message has empty prev} \
    {*}$ms_common \
    -body {
        ms_batch [list [ms_msg timestamp 42 body only]]
        set msg [lindex [store get latest alice@example.com] 0]
...
test messagestore-prev-around-chains-across-boundary {get around chains prev across before/target/after} \
    {*}$ms_common \
    -body {
        ms_batch [list \
            [ms_msg timestamp 100 body a] \
        ...
        set result [store get around alice@example.com 300 4]
        set msgs [dict get $result messages]
        # [a, b, c, d, e]; c is target, a-b before, d-e after
        list [dict get [lindex $msgs 0] prev] \
             [dict get [lindex $msgs 1] prev] \
             [dict get [lindex $msgs 2] prev] \
             [dict get [lindex $msgs 3] prev] \
             [dict get [lindex $msgs 4] prev]
    } -result {{} 100 200 300 400}

test messagestore-prev-region-boundary {prev does not leak across region gap} \
    {*}$ms_common \
    -body {
        ms_batch [list \
            [ms_msg timestamp 100 body a] \
        ...
        ms_batch [list \
            ...
            [ms_msg timestamp 600 body d]]
        set msgs [store get latest alice@example.com]
        # latest region is [c, d]; c's prev should be empty (no predecessor in its region)
        list [dict get [lindex $msgs 0] prev] \
             [dict get [lindex $msgs 1] prev]
    } -result {{} 500}

test messagestore-prev-empty-result {empty result returns empty list, no error} \
    {*}$ms_common \
    -body {
        store get latest nobody@example.com
    } -result {}
